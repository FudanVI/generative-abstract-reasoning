...
class VAPDataset(Dataset):
    ...
    def __init__(self, size, cache_root='cache', name='transfer', part='train'):
        super(VAPDataset, self).__init__()
        self.name = name
        self.part = part
        self.size = size
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            imgs = [imgs[:, :, i * 160:(i + 1) * 160] for i in range(9)]
            imgs = torch.stack(imgs, dim=0)
            imgs = interpolate(imgs, self.size, mode='bilinear', align_corners=False)
            panel, candidates = imgs[:5], imgs[5:]
