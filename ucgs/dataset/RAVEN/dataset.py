...
class RAVENDataset(Dataset):
    ...
    def __init__(self, size, cache_root='cache', name='center_single', part='train'):
        super(RAVENDataset, self).__init__()
        self.name = name
        self.part = part
        self.size = size
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            imgs = [imgs[:, :, i*160:(i+1)*160] for i in range(16)]
            imgs = torch.stack(imgs, dim=0)
            imgs = interpolate(imgs, self.size, mode='bilinear', align_corners=False)
            panel, candidates = imgs[:8], imgs[8:]
            answer_key = 'answer-%09d' % index
            answer_idx = int(str(txn.get(answer_key.encode()).decode('utf-8')))
            answer_idx = torch.tensor(answer_idx).long()
            indices = torch.tensor(list(range(candidates.size(0)))).long()
            distractors = candidates[indices != answer_idx]
            answer = candidates[indices == answer_idx]
