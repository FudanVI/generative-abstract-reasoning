...
from backbone_vqvae.main import MainNet
...
def prepare(context):
    ...
    optimizer = Adam(model.backbone.parameters(), args.lr)
    context['model'] = model
...
def train_epoch(context):
    ...
    for key, val in train_log_loss.items():
        context['writer'].add_scalar(
            'train_loss/{}'.format(key), sum(val) / len(val), context['epoch'] + 1)
...
def eval_epoch(context):
    ...
    if val_loss < context['best_metric']:
        context['best_metric'] = val_loss
    ...
    print('====> Best Loss = {:F} @ Epoch {}'.format(context['best_metric'], context['best_epoch']))
...
def visualize(context):
    ...
    with torch.no_grad():
        ...
        panel_recon = results['panel_recon'][:32]
        panel = results['panel'][:32]
        vis_panel = torch.cat([panel, panel_recon], dim=0)
        writer.add_image('recon_panels', make_grid(vis_panel, nrow=8), epoch + 1)
