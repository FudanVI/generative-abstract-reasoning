...
class Normal:
    ...
    def kld(self, params, params_prior, clamp=False):
        mean, std = self.unpack_params(params)
        mean_prior, std_prior = self.unpack_params(params_prior)
        if clamp:
            std = std.clamp_min(1e-5)
            std_prior = std_prior.clamp_min(1e-5)
...
class MultivariateNormal:
    ...
    @staticmethod
    def filter_nan_inf(t):
        t = torch.where(t != t, t.new_zeros(t.size()), t)
        t = torch.where(torch.isinf(t), t.new_zeros(t.size()), t)
        return t

    def log_density(self, sample, mean, p):
        log_cov_det = torch.logdet(p)
        vec = (sample - mean)[..., None]
        pow2 = vec.transpose(-2, -1).matmul(p).matmul(vec).reshape(log_cov_det.size())
        log_prob = - 0.5 * (log_cov_det + pow2)
        log_prob = self.filter_nan_inf(log_prob)
        return log_prob
